function searchForWorks(
  key,
  grpIdx = 0,
  isCache = 1,
  mode = tmpSearchSettings["pixivbiu_searchMode"]
) {
  cssShowLoading();
  setTimeout(
    "progresserSearching('" +
      key +
      "_" +
      String(tmpSearchSettings["pixivbiu_searchPageNum"]) +
      "+" +
      String(grpIdx) +
      "')",
    200
  );
  if (mode !== "tag" && mode !== "otag" && mode !== "des") mode = "tag";
  $.ajax({
    type: "GET",
...
function getMarks(user = "", mode = "public", grp = "0@0") {
  ...
  $.ajax({
    ...
    success: function (rep) {
      ...
      if (rep.code) {
        if (
          grpIdx === grpArr.length - 1 &&
          rep["msg"]["args"]["ops"]["markNex"] != "None"
        )
          rep["msg"]["args"]["ops"]["tmp"] =
            grp.split("@")[0] +
            "@" +
            grp.split("@")[1] +
            "_" +
            rep["msg"]["args"]["ops"]["markNex"];
        tmpPageData = rep.msg;
        if (user === "" || user === "my") {
...
function doDownloadPic(kt, workID = 0, idx = -1) {
  if (downloadList.hasOwnProperty(workID)) return;

  let data;
  if (idx !== -1) data = JSON.stringify(tmpPageData["rst"]["data"][idx]["all"]);
  else data = 0;
